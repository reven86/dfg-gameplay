...
public abstract class DownloaderService extends CustomIntentService implements IDownloaderService {
            ...
            case ConnectivityManager.TYPE_MOBILE:
                ...
                switch (subType) {
                    ...
                    case TelephonyManager.NETWORK_TYPE_UMTS:
                        ...
                        mIsAtLeast4G = false;
                        break;
                    case 13: //TelephonyManager.NETWORK_TYPE_LTE: // 4G
                    case 14: //TelephonyManager.NETWORK_TYPE_EHRPD: // 3G ++ interop
                                                              // with 4G
                    case 15: //TelephonyManager.NETWORK_TYPE_HSPAP: // 3G ++ but
                                                              // marketed as
                                                              // 4G
                        mIsAtLeast3G = true;
                        mIsAtLeast4G = true;
    ...
    private static boolean isLVLCheckRequired(DownloadsDB db, PackageInfo pi) {
        ...
        // proceed
        if (db.mVersionCode != pi.versionCode || db.getDownloads() == null) {
            return true;
        }
        return false;
    }
    ...
    public static int startDownloadServiceIfRequired(Context context,
        ...
        //Log.e(Constants.TAG, "db status: " + Integer.toString(db.mStatus));
        // we don't have to update LVL. do we still have a download to start?
        if (db.mStatus == 0) {
            DownloadInfo[] infos = db.getDownloads();
            if (null != infos) {
                for (DownloadInfo info : infos) {
                    Log.e(Constants.TAG, info.mFileName);
